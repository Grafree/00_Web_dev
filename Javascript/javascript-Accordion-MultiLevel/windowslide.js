 ...
 * @param idmenu			[string] id des fenetres coulissantes 
 * @param deccStart			[int] Pourcent de deceleration de l'animation
 * @param openState			[bolean] fermeture automatique des fenetres coulissantes a l'ouverture d'une autre = true
 ...
 * @param minHeightSlide	[int] Nb de px des fenetres fermees. Correspond a la propriete CSS height de la classe slider_content
 * @param paddingInnerSlide	[int] Correspond au padding indique dans le CSS pour content_show
 * @return void
 */
function accordionWindow( distanceInt, speedInt, idmenu, deccStart, openState, openStartState, openSlideState, minHeightSlide, paddingInnerSlide ){
	...
	var speed = speedInt; 				// vitesse en milliseconde. Plus la valeur est eleve plus l'animation sera lente.
	var id = idmenu;

	var start = deccStart; 				// Pourcentage a partir duquel la deceleration commence (ex. 20, signifie a 20% de la fin) 0 indique aucune deceleration
	var automatic = openState;			// indique si la fermeture des slides est automatique (true) a l'ouverture d'un entre eux.
	var startOpen = openStartState; 	// Id de la slide qui devrait etre ouverte sans coulissement au chargement. 0 signifie aucune ouverture au depart.
	...
	var slideHeightMin = minHeightSlide;// Ouverture de base en px de la fenetre coulissante. Doit correspondre a la propriete height de la classe slider_content
	var padding = paddingInnerSlide; 	// Doit correspondre au padding indiquee dans le CSS pour content_show

	var current_slider = 0;			// Indique la fenetre coulissante actuellement ouverte
	var current_slider_autoclose = 0;	// Indique la fenetre coulissante a fermer
	...
	var SubAndParentHeight = 0;			// Indique la hauteur combine du sous-element et de la fenetre
	...
	this.action = function( slideId ){
		...
		if ( min == slideHeightMin ) { // open the window
			document.getElementById('current').innerHTML = slideId;
			...
			if (document.getElementById('current').innerHTML == slideId) {
	...
	this.entete_roll = function( slideId ){
		classItem = document.getElementById( slideId ).className;
		document.getElementById( slideId ).className = classItem + '_roll';
	}
	this.entete_normal = function( slideId ){
		document.getElementById( slideId ).className = classItem;
	}
